...
def _to_iplot(self, colors=None, colorscale=None,
              kind='scatter', mode='lines', interpolation='linear',
	...
	bestfit : boolean or list
        If True then a best fit line will be generated for all columns. 
        If list then a best fit line will be generated for each key on the list. 
    ...
    for key in keys:
        ...
        if text is not None:
            lines[key]["text"] = text
        if 'bar' in kind:
            lines[key]["marker"] = {'color': to_rgba(colors[key], opacity), 'line': {
...
def _iplot(self, kind='scatter', data=None, layout=None, filename='', sharing=None,
	...
	margin : dict or tuple
		...
		Tuple containing the left, right, bottom and top margins
	labels : string
	...
	subplots : bool
		If true then each trace is placed in subplot layout
	shape : (rows,cols)
    ...
    FF_DISTPLOT = ["group_labels", "bin_size", "curve_type",
                   "rug_text", "show_hist", "show_curve", "show_rug"]
	...
	#region Get values from config theme
    if theme is None:
    ...
    #region In case column was used instead of keys
    if 'column' in kwargs:
    ...
    #region We assume we are good citizens
    validate = True
    ...
    if not layout:
        ...
        if annotations:
            ...
            annotations = tools.get_annotations(
        ...
        if categories and kind not in ('violin',):
                ...
                for _ in _keys:
                    ...
                    if 'bubble' in kind:
                        ...
                        if not kwargs.get('abs_size', False):
                            if len(rgo) > 1:
                                _size = [
                                    int(100*(float(i)-rgo.min())/(rgo.max()-rgo.min()))+12 for i in rg]
                            else:
            ...
            if kind in ('scatter', 'spread', 'ratio', 'bar', 'barh', 'area', 'line'):
                ...
                if y and secondary_y:
                    ...
                    _secondary_y = [secondary_y] if not isinstance(
                        secondary_y, list) else secondary_y
                ...
                for _ in columns:
                    if kind == 'box':
                        ...
                        __ = dict(x=df[_].values.tolist(), name=_,
                                  marker=dict(
                                      color=clrs[_], line=dict(width=width)),
                ...
                if kind == 'heatmap':
                    data = [Heatmap(z=z, x=x, y=y, zmin=zmin,
                                    zmax=zmax, colorscale=colorscale)]
                else:
                ...
                marker = dict(colors=get_colors(
                    colors, colorscale, labels, asList=True))
                marker.update(line=dict(color=kwargs.pop(
                    'linecolor', None), width=kwargs.pop('linewidth', width)))
                pie = dict(values=values, labels=labels,
                           name='', marker=marker)
                ...
                _d['increasing'] = dict(
                    line=dict(color=kw['up_color']) if 'up_color' in kw else dict())
                _d['decreasing'] = dict(
                    line=dict(color=kw['down_color']) if 'down_color' in kw else dict())
            ...
            # Figure Factory
                ...
                colors = get_colors(colors, colorscale, df[categories].value_counts(
                ).values if categories else df.keys(), asList=True)
                kw['colors'] = colors
                if categories:
                    ...
                    if categories not in df:
                        raise CufflinksError(
                            'Column "{0}" not found in DataFrame'.format(categories))
    ...
    #region Figure defintion
    figure = {}
    figure['data'] = data
    figure['layout'] = layout
    #endregion

    #region Check secondary axis
    if secondary_y:
    ...
    if subplots:
        ...
        for _ in ['x', 'y']:
            if 'shared_{0}axes'.format(_) not in kw:
                kw['shared_{0}axes'.format(_)] = kw.pop(
                    'shared_{0}axis'.format(_), False)
