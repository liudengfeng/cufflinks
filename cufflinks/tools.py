...
def getLayout(kind=None, theme=None, title='', xTitle='', yTitle='', zTitle='',
              barmode='', bargap=None, bargroupgap=None,
    ...
    if is3d:
        ...
        scene = dict(xaxis=layout['xaxis'].copy(),
                     yaxis=layout['yaxis'].copy(), zaxis=zaxis)
    ...
    def check_ann(annotation):
        ...
        if 'title' in annotation:
            ...
            ann = dict(
                ...
                arrowcolor=annotation.get(
                    'arrowcolor', kwargs.get('arrowcolor')),
                ...
                font=dict(
                    color=annotation.get('fontcolor', annotation.get(
                        'color', kwargs.get('fontcolor'))),
                    size=annotation.get('fontsize', annotation.get(
...
def get_subplots(rows=1, cols=1,
    ...
    layout = base_layout if base_layout else getLayout(
        theme, **check_kwargs(kwargs, __LAYOUT_AXIS))
...
def get_shape(kind='line', x=None, y=None, x0=None, y0=None, x1=None, y1=None,
              span=0, color='red', dash='solid', width=1,
...
def get_range_selector(steps=['1m', '1y'], 
...
def get_error_bar(axis='y', type='data', values=None, values_minus=None, 
                  color=None, thickness=1, width=5,
                  opacity=1):
    ...
    if type == 'data':
        if isinstance(values, list) or isinstance(values, np.ndarray):
            ...
            if values_minus:
                ...
                error.update(symmetric=False, valueminus=values_minus)
...
def set_errors(figure, trace=None, axis='y', type='data', values=None, values_minus=None, 
               color=None, thickness=1, width=None,
               opacity=None, **kwargs):
    figure = fig_to_dict(figure)
    if 'value' in kwargs:
        values = kwargs['value']
    ...
    if 'continuous' not in type:
        ...
        def get_traces(trace, value, type, color=None, width=.2, opacity=.3):
            ...
            if values_minus:
                ...
                min_value = value
            if 'percent' in type:
                y_up = [trace['y'][_]*(1+value[_]/100.00)
                        for _ in range(len(value))]
                y_down = [trace['y'][_]*(1-min_value[_]/100.00)
                          for _ in range(len(min_value))]
            else:
                y_up = [trace['y'][_]+value[_] for _ in range(len(value))]
                y_down = [trace['y'][_]-min_value[_]
                          for _ in range(len(min_value))]
