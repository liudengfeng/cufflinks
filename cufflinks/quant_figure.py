...
class QuantFig(object):
	...
	def __init__(self, df, kind='candlestick', columns=None, **kwargs):
		...
		self.data.update(
			datalegend=kwargs.pop('datalegend', True),
			name=kwargs.pop('name', 'Trace 1'), kind=kind)
		self.data.update(
			slice=kwargs.pop('slice', (None, None)),
		...
		self.theme['up_color'] = kwargs.pop('up_color', 'red')  # java
	...
	def _get_sliced(self, slice, df=None, to_strfmt='%Y-%m-%d', from_strfmt='%d%b%y'):
		...
		# 🆗 不使用 to_strfmt from_strfmt
		# if a:
		#     a = date_tools.stringToString(
		#         a, from_strfmt, to_strfmt) if '-' not in a else a
		# if b:
		#     b = date_tools.stringToString(
		#         b, from_strfmt, to_strfmt) if '-' not in b else b
    ...
    def _get_trendline(self, date0=None, date1=None, on=None, kind='trend', to_strfmt='%Y-%m-%d', from_strfmt='%d%b%y', **kwargs):
		...
		Parameters:
			...
			to_strfmt : string
				...
				default : '%Y-%m-%d'
		"""
		start_loc, end_loc = kwargs.pop('start_loc'), kwargs.pop('end_loc')
		...
		x0, x1 = self.get_display_locs(date0, date1)

		if kind == 'trend':
		    on = 'close' if not on else on
			y0 = kwargs.get('y0', self.df[self._d[on]][x0])  # # 修改
			y1 = kwargs.get('y1', self.df[self._d[on]][x1])  # # 修改
		...
		if kind in ('support', 'resistance'):
		    ...
		    if mode == 'starttoend':
				...
				x1 = start_loc  # # 修改
				y0 = kwargs.get('y0', self.df[self._d[on]][x0])  # # 修改
				y1 = kwargs.get('y1', y0)  # # 使用y0值

		d = {'x0': x0, 'x1': x1, 'y0': y0, 'y1': y1}
		d.update(**kwargs)
		shape = tools.get_shape(**d)

		if ann_kwargs.get('text', False):
			default_x = position(x0, x1)
	...
	def add_annotations(self, annotations, **kwargs):
		...
		"""
		x, _ = self.get_display_locs(date)
		on = 'high' if not on else on
		y = kwargs.get('y0', self.df[self._d[on]][x])
		annotation = dict(
			x=x,
			y=y,
			xref='x',
		...
		self.layout['annotations']['values'].append(annotation)
		ann_kwargs = utils.check_kwargs(
	...
	def get_display_locs(self, start=None, end=None):
		"""自切片获取原始数据框的开始及结束位置"""
		if start is None:
			start_loc = 0
		else:
			start_loc = self.df.index.get_loc(start, 'bfill')  # 最接近结束日期的大值
		if end is None:
			end_loc = len(self.df) - 1  # 位置自0开始
		else:
			end_loc = self.df.index.get_loc(end, 'ffill')   # 最接近结束日期的小值
	...
	if kind == 'boll':
		...
		fillcolor = local_kwargs.pop(
		    'fillcolor', fig['data'][2]['line']['color'] or 'rgba(200,200,200,.1)')
	...
	def _check_date(self, d0, d1):
		...
		if d1 is None:
			d1 = end_date
		start_loc, end_loc = self.get_display_locs(start_date, end_date)
		loc0, loc1 = self.get_display_locs(d0, d1)
		if loc0 < start_loc:
	...
	self_kwargs = copy.deepcopy(self.kwargs)
		...
		self.display_end_date = end_date
		# # 位置切片
		start_loc, end_loc = self.get_display_locs(start_date, end_date)
	...
	d = self_kwargs
		# # 输入全部数据。主要目的在于可以在计算技术指标后，然后选择有效时段的数据绘图。
		df = self.df.copy()
	if _resample:
	    if utils.is_list(_resample):
		df = self._get_resampled(*_resample, df=df)
	...
	for trend in self.trendlines:
			d0, d1 = trend.get('date0'), trend.get('date1')
			self._check_date(d0, d1)
			trend.update(dict(start_loc=start_loc, end_loc=end_loc))
	...
	if d['kind'] not in ('candle', 'candlestick', 'ohlc'):
		...
		# # 添加shapes、annotations
		fig['layout']['shapes'] = shapes['shapes']
		fig['layout']['annotations'] = annotations['values']
	...
	if showstudies:
		...
		if v['kind'] in ('rsi', 'volume', 'macd', 'atr', 'adx', 'cci', 'dmi'):
		    max_panel += 1
		    panel_data['n'] += 1
		    tools._move_axis(study_fig, yaxis='y{0}'.format(
			max_panel))  # FIXME TKP
